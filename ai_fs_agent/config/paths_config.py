...
def ensure_directories() -> None:
    """确保所有必要目录存在"""
    for d in [LOCAL_CONFIG_DIR, DATA_DIR, LOGS_DIR, RAG_INDEX_DIR]:
...
def ensure_files() -> None:
    ...
    if not ENV_PATH.exists():
        from ai_fs_agent.config.templates import ENV_TEMPLATE

        ENV_PATH.write_text(ENV_TEMPLATE, encoding="utf-8")

    # llm_models.toml
    if not LLM_CONFIG_PATH.exists():
        from ai_fs_agent.config.templates import LLM_CONFIG_TEMPLATE
